<!-----------------------------------------------------------------------------------
--
-- Carousel
--
------------------------------------------------------------------------------------->
<div class="container">
    <div id="medlab-carousel" class="carousel slide medlab_carousel" data-ride="carousel">

        <ol class="carousel-indicators">
            <li data-target="#medlab-carousel" data-slide-to="0" class="active"></li>
            <li data-target="#medlab-carousel" data-slide-to="1"></li>
            <li data-target="#medlab-carousel" data-slide-to="2"></li>
            <li data-target="#medlab-carousel" data-slide-to="3"></li>
            <li data-target="#medlab-carousel" data-slide-to="4"></li>
            <li data-target="#medlab-carousel" data-slide-to="5"></li>
            <li data-target="#medlab-carousel" data-slide-to="6"></li>
            <li data-target="#medlab-carousel" data-slide-to="7"></li>
        </ol>

        <div class="carousel-inner" role="listbox">
            <div class="item active">
                <img class="medlab_carousel_image" src="/img/carousel/1.jpg" alt="1">
                <div class="carousel-caption"></div>
            </div>
            <div class="item">
                <img class="medlab_carousel_image" src="/img/carousel/2.jpg" alt="2">
                <div class="carousel-caption"></div>
            </div>
            <div class="item">
                <img class="medlab_carousel_image" src="/img/carousel/3.jpg" alt="3">
                <div class="carousel-caption"></div>
            </div>
            <div class="item">
                <img class="medlab_carousel_image" src="/img/carousel/4.jpg" alt="4">
                <div class="carousel-caption"></div>
            </div>
            <div class="item">
                <img class="medlab_carousel_image" src="/img/carousel/5.jpg" alt="5">
                <div class="carousel-caption"></div>
            </div>
            <div class="item">
                <img class="medlab_carousel_image" src="/img/carousel/6.jpg" alt="6">
                <div class="carousel-caption"></div>
            </div>
            <div class="item">
                <img class="medlab_carousel_image" src="/img/carousel/7.jpg" alt="7">
                <div class="carousel-caption"></div>
            </div>
            <div class="item">
                <img class="medlab_carousel_image" src="/img/carousel/8.jpg" alt="7">
                <div class="carousel-caption"></div>
            </div>
        </div>

    </div>
</div>

<!-----------------------------------------------------------------------------------
--
-- Carousel Bottom Panel
--
------------------------------------------------------------------------------------->
<div class="container">
    <table class="medlab_carousel_bottom_panel_table">
        <tbody>
        <tr>
            <td class="medlab_carousel_bottom_panel_column1">Research / Innovation</td>
            <td class="medlab_carousel_bottom_panel_column2">100% Satisfaction</td>

            <td class="medlab_carousel_bottom_panel_column3">
                <a href="#">
                    <div class="medlab_carousel_bottom_panel_column3_link">Ask a Question!</div>
                </a>
            </td>
        </tr>
        </tbody>
    </table>
</div>